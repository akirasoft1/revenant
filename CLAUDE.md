...
**Pre-roll buffer:** while idle, `VoiceService` keeps a rolling ~3s pre-roll (`MAX_PREROLL_FRAMES`) of 16k PCM. On wake it captures that buffer (plus any audio arriving during the ~3s session-open startup) into `g.pending` and flushes it into the Live session right after `sendStart`, so a first-turn question spoken in the same breath as the wake phrase ("hey jarvis, what's 2+2") isn't lost in the wake→session gap. The wake-phrase audio is sent through too (harmless; reads as address). **`g.pending` is bounded** at `MAX_PENDING_FRAMES` (~5s, drop-oldest, same shape as the pre-roll): the open awaits `_contextBuilder` with no timeout, and an unbounded buffer would flush minutes of backlog into the model as if it were current speech — the same reason the reconnect path drops frames instead of buffering them.

**Session-open failure is recoverable:** everything after `_startSession`'s health gate runs inside a try/catch. Any throw (e.g. `sendStart` on a duplex grpc-js has destroyed) routes through `_resetToIdle` — the one definition of "put this guild back where a wake word works again" — which ends the session, clears buffers/floor/gates and installs a fresh `VoiceSessionMachine`. `_tick` carries a matching recovery for any guild found non-idle with no open session, deliberately NOT gated on `g.session` (that is exactly what a failed open never sets); `g.sessionOpening` keeps it from tearing down a session that is merely slow to open. The same reset handles a **clean sidecar-side stream close** (`session.on('end')`): the sidecar exits silently on two paths (no resumption handle, and a normal close classified `outcome = "closed"`), so `'end'` is often the ONLY signal the bot gets that its stream is dead.

**The `/voice` replies say what actually happened:** `join()` returns `{joined, reason, channelId}` and `listen()` returns `{listening, reason, channelId}`, and the command renders the refusals (already in a different channel, a session already active, no session opened because the sidecar was unhealthy) instead of reporting success for all of them. A `/voice listen` reply also names the `VOICE_MAX_SESSION_SECONDS` cap, because that cap — not `/voice leave` — is what usually ends listen mode (600s default, not overridden in the deployed overlay); when it fires, `_noteContinuousEnded` logs that continuous listen has ended and a wake word is required again. `leave()` deletes the guild entry in a `finally`, so a throwing teardown can no longer make voice permanently unavailable for that guild.
