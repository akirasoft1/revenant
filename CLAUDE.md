...
**K8s API timeouts (`k8s_client.py`):** every kubernetes-client call carries an explicit `_request_timeout` (default `(5, 30)`). Without it an API server that accepts the connection and goes silent parks the `asyncio.to_thread` worker forever AND holds the orchestrator's `ConcurrencyGate` permit — permits are global and finite, so repeated hangs walked the sandbox to a standstill only a restart cleared. `SandboxOrchestrator.run` additionally bounds one execution (`overall_deadline_s`, default `wall_clock + 120`) inside the gate as a backstop, returning `orchestrator_error="orchestrator_timeout"`; an abandoned Job cleans itself up via `activeDeadlineSeconds` + `ttlSecondsAfterFinished`.
