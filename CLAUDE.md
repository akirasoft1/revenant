...
**Deferred follow-ups (not yet implemented):** idle auto-leave (channel currently requires `/voice leave`), per-speaker transcript author fields (multi-user voice channels currently attribute transcripts without per-speaker identity), dynamic (session-time, non-static) voice-profile injection into the Live system prompt, and playback jitter smoothing (occasional; likely a jitter buffer + proper 24k→48k resampler).
