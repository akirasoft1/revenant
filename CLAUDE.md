...
**Deferred follow-ups (not yet implemented):** idle auto-leave (channel currently requires `/voice leave`), dynamic (session-time, non-static) voice-profile injection into the Live system prompt, playback jitter smoothing (occasional; likely a jitter buffer + proper 24k→48k resampler), model identity-awareness in the prompt (Phase 3 — currently the floor-holder's name appears in transcript attribution but not in system context to the model), and human-like deferral responses (Phase 4 — will leverage `FloorControl.waiting()` to craft replies acknowledging other speakers when they queue).
