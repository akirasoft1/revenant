...
**OTel tracing:** the agent sidecar exports OpenTelemetry spans to the in-cluster Dynatrace collector (`telemetry-ingest.dynatrace.svc:4318`, OTLP/HTTP) as service `discord-article-bot-agent` — ADK's native `agent_run → call_llm → execute_tool` waterfall, with LLM message content captured (`ADK_CAPTURE_MESSAGE_CONTENT_IN_SPANS=true`). Configured via `OTEL_EXPORTER_OTLP_ENDPOINT` on the agent deployment; `agent-networkpolicy.yaml` allows the collector egress (sandbox-job isolation via `sandbox-networkpolicy.yaml` is unaffected). `tracing.py` is no-op when the endpoint env is unset (local dev). The agent also instruments its gRPC server so its spans nest under the bot's `Agent/Chat` span. Query via dtctl: `fetch spans | filter dt.service.name == "discord-article-bot-agent"`.
