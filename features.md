...
- **Visible Degradation**: A fallback is announced rather than silent. Falling through to direct OpenAI names the model that answered instead, and a turn that ran without the channel-voice personality or memory context says so, in the reply and in the logs.
