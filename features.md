...
- **Deferred**: idle auto-leave, dynamic (non-static) voice-profile injection into the Live system prompt, session compression/resumption for longer conversations (Plan 2b), self-service speaker names via `/voice name` command so users set their own preferred spoken names (Phase 4), and human-like deferral responses acknowledging waiting speakers (Phase 4)
